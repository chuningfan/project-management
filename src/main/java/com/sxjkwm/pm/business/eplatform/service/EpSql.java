...
public class EpSql {
    ...
    static String outboundInvoiceSql = "SELECT DISTINCT o.supplier_org_id, o.supplier_name, o.buyer_name, o.order_time, o.buyer_org_name, o.buyer_org_full_name, o.sale_order_no, o.final_price, bi.bill_code, bi.bill_price, ia.apply_number, " +
            ...
            " AND NOT EXISTS (SELECT 1 FROM service_order.order_info ooi WHERE ooi.parent_no = oi.order_no AND ooi.parent_no > 0) " +
            ") o " +
            " LEFT JOIN service_order.`bill_order` bo ON bo.order_id = o.sale_order_no " +
            " LEFT JOIN service_order.`bill_infos` bi ON bi.id = bo.bill_id " +
    ...
    static String inboundInvoiceSql = " SELECT DISTINCT o.supplier_org_id, o.org_name supplier_name, o.buyer_org_name, o.buyer_org_full_name, o.buy_order_no, o.final_price, " +
            ...
            " FROM service_order.`bill_order` bo" +
            " INNER JOIN service_order.`order_third_relation` otr ON otr.third_order_no = bo.order_id AND otr.`yn` = 1" +
            " INNER JOIN service_order.`bill_infos` bi ON bi.id = bo.`bill_id` AND bi.`yn` = 1 AND bi.has_reseller_create IS NULL AND bi.`status` = 2 " +
            ") bb ON bb.order_no = o.buy_order_no " +
            " LEFT JOIN (" +
            " SELECT otr.`order_no` AS bi_order_no, ia.`apply_number` AS bi_invoice_apply_number, ia.`apply_status` AS bi_invoice_apply_status, " +
            " ia.`tatal_amount` AS bi_amount, ia.`billing` AS bi_billing, ia.`cancel_status` AS bi_invoice_cancel_status, ia.invoice_type AS bi_invoice_type, ia.invoice_num, ia.modified" +
            " FROM service_order.`invoice_apply_order_relation` iaor" +
            " INNER JOIN service_order.`invoice_apply` ia ON ia.id = iaor.`invoice_apply_id` AND ia.`sql_type` = 0 AND ia.`yn` = 1 AND ia.`apply_status` = 1 AND ia.`deliver_type` = 1" +
            " INNER JOIN service_order.`order_third_relation` otr ON otr.`third_order_no` = iaor.`order_id` AND otr.`yn` = 1 " +
            ") bi ON bi.bi_order_no = o.buy_order_no " +
    ...
    static String invoiceSql = "SELECT\n" +
            ...
            "\ta6.invoice_title AS invoiceTitle,\n" +
            "\ta7.NAME AS organizeName\n" +
            "FROM\n" +
            "\tservice_order.invoice_apply a2\n" +
            "\tLEFT JOIN service_order.invoice_apply_invoice a1 ON a2.id = a1.invoice_apply_id\n" +
            ...
            "\tLEFT JOIN service_order.invoice_apply_order_relation a6 ON a2.id = a6.invoice_apply_id\n" +
            "\tLEFT JOIN service_user.organization a7 ON a2.organize_id = a7.id \n" +
            "WHERE\n" +
            "\ta2.apply_status = 1 \n" +
            "\tAND a2.billing = 1 \n" +
            "\tAND ISNULL( a2.cancel_status ) \n" +
            "\tAND a2.sql_type = 1 GROUP BY a3.order_no";
